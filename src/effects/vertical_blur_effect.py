...
class VerticalBlurEffect(Material):
    ...
    def __init__(self, texture_size=[512, 512], blur_radius=20) -> None:
        ...
        void main()
        {
            ...
            for (int offset_y = -u_blurRadius; offset_y <= u_blurRadius; offset_y++)
            {
                float weight = u_blurRadius - abs(offset_y) + 1;
                vec2 offset_tex_coord = vec2(0, offset_y) * pixel_to_tex_coord;
